import { CommonModule } from '@angular/common'
import {
  Component,
  ElementRef,
  QueryList,
  ViewChildren,
  ChangeDetectorRef,
  inject,
  computed,
} from '@angular/core'
import { SidebarService } from '../../services/sidebar.service'
import { NavigationEnd, Router, RouterModule } from '@angular/router'
import { SafeHtmlPipe } from '../../pipe/safe-html.pipe'
import { combineLatest, Subscription } from 'rxjs'
import { AuthService } from '@/core/auth/auth.service'

type NavItem = {
  name: string
  icon: string
  path?: string
  new?: boolean
  modulo?: string
  adminOnly?: boolean
  subItems?: {
    name: string
    path: string
    pro?: boolean
    new?: boolean
    modulo?: string
  }[]
}

@Component({
  selector: 'app-sidebar',
  imports: [CommonModule, RouterModule, SafeHtmlPipe],
  templateUrl: './app-sidebar.component.html',
})
export class AppSidebarComponent {
  private readonly auth = inject(AuthService)

  // Main nav items
  private readonly allNavItems: NavItem[] = [
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M5.5 3.25C4.25736 3.25 3.25 4.25736 3.25 5.5V8.99998C3.25 10.2426 4.25736 11.25 5.5 11.25H9C10.2426 11.25 11.25 10.2426 11.25 8.99998V5.5C11.25 4.25736 10.2426 3.25 9 3.25H5.5ZM4.75 5.5C4.75 5.08579 5.08579 4.75 5.5 4.75H9C9.41421 4.75 9.75 5.08579 9.75 5.5V8.99998C9.75 9.41419 9.41421 9.74998 9 9.74998H5.5C5.08579 9.74998 4.75 9.41419 4.75 8.99998V5.5ZM5.5 12.75C4.25736 12.75 3.25 13.7574 3.25 15V18.5C3.25 19.7426 4.25736 20.75 5.5 20.75H9C10.2426 20.75 11.25 19.7427 11.25 18.5V15C11.25 13.7574 10.2426 12.75 9 12.75H5.5ZM4.75 15C4.75 14.5858 5.08579 14.25 5.5 14.25H9C9.41421 14.25 9.75 14.5858 9.75 15V18.5C9.75 18.9142 9.41421 19.25 9 19.25H5.5C5.08579 19.25 4.75 18.9142 4.75 18.5V15ZM12.75 5.5C12.75 4.25736 13.7574 3.25 15 3.25H18.5C19.7426 3.25 20.75 4.25736 20.75 5.5V8.99998C20.75 10.2426 19.7426 11.25 18.5 11.25H15C13.7574 11.25 12.75 10.2426 12.75 8.99998V5.5ZM15 4.75C14.5858 4.75 14.25 5.08579 14.25 5.5V8.99998C14.25 9.41419 14.5858 9.74998 15 9.74998H18.5C18.9142 9.74998 19.25 9.41419 19.25 8.99998V5.5C19.25 5.08579 18.9142 4.75 18.5 4.75H15ZM15 12.75C13.7574 12.75 12.75 13.7574 12.75 15V18.5C12.75 19.7426 13.7574 20.75 15 20.75H18.5C19.7426 20.75 20.75 19.7427 20.75 18.5V15C20.75 13.7574 19.7426 12.75 18.5 12.75H15ZM14.25 15C14.25 14.5858 14.5858 14.25 15 14.25H18.5C18.9142 14.25 19.25 14.5858 19.25 15V18.5C19.25 18.9142 18.9142 19.25 18.5 19.25H15C14.5858 19.25 14.25 18.9142 14.25 18.5V15Z" fill="currentColor"></path></svg>`,
      name: 'Dashboard',
      path: '/dashboard',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 3.5C7.30558 3.5 3.5 7.30558 3.5 12C3.5 14.1526 4.3002 16.1184 5.61936 17.616C6.17279 15.3096 8.24852 13.5955 10.7246 13.5955H13.2746C15.7509 13.5955 17.8268 15.31 18.38 17.6167C19.6996 16.119 20.5 14.153 20.5 12C20.5 7.30558 16.6944 3.5 12 3.5ZM17.0246 18.8566V18.8455C17.0246 16.7744 15.3457 15.0955 13.2746 15.0955H10.7246C8.65354 15.0955 6.97461 16.7744 6.97461 18.8455V18.856C8.38223 19.8895 10.1198 20.5 12 20.5C13.8798 20.5 15.6171 19.8898 17.0246 18.8566ZM2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12ZM11.9991 7.25C10.8847 7.25 9.98126 8.15342 9.98126 9.26784C9.98126 10.3823 10.8847 11.2857 11.9991 11.2857C13.1135 11.2857 14.0169 10.3823 14.0169 9.26784C14.0169 8.15342 13.1135 7.25 11.9991 7.25ZM8.48126 9.26784C8.48126 7.32499 10.0563 5.75 11.9991 5.75C13.9419 5.75 15.5169 7.32499 15.5169 9.26784C15.5169 11.2107 13.9419 12.7857 11.9991 12.7857C10.0563 12.7857 8.48126 11.2107 8.48126 9.26784Z" fill="currentColor"></path></svg>`,
      name: 'Utilizadores',
      path: '/users',
      adminOnly: true,
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12.5 2.75a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-.75v1.04c2.51.24 4.5 2.35 4.5 4.96 0 1.38-.56 2.63-1.46 3.53l1.58 1.58a.75.75 0 1 1-1.06 1.06l-1.58-1.58A4.98 4.98 0 0 1 13 14.5v4.75h.75a.75.75 0 0 1 0 1.5h-3.5a.75.75 0 0 1 0-1.5h.75V14.5a4.98 4.98 0 0 1-3.47-1.66l-1.58 1.58a.75.75 0 1 1-1.06-1.06l1.58-1.58A4.98 4.98 0 0 1 5.5 9.5c0-2.61 1.99-4.72 4.5-4.96V3.5h-.75a.75.75 0 0 1 0-1.5h2.5a.75.75 0 0 1 .75.75ZM10 6.5A3.5 3.5 0 0 0 7 10a3.5 3.5 0 1 0 7 0A3.5 3.5 0 0 0 10 6.5Z" fill="currentColor"/></svg>`,
      name: 'Roles',
      path: '/roles',
      adminOnly: true,
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M6 2.75A2.25 2.25 0 0 0 3.75 5v14A2.25 2.25 0 0 0 6 21.25h12A2.25 2.25 0 0 0 20.25 19V8.31a2.25 2.25 0 0 0-.66-1.59l-3.31-3.31a2.25 2.25 0 0 0-1.59-.66H6ZM5.25 5A.75.75 0 0 1 6 4.25h8V7a2.25 2.25 0 0 0 2.25 2.25h2.5V19a.75.75 0 0 1-.75.75H6a.75.75 0 0 1-.75-.75V5Zm10.25-.44 2.94 2.94H16.25A.75.75 0 0 1 15.5 6.75V4.56ZM8 12.25a.75.75 0 0 0 0 1.5h8a.75.75 0 0 0 0-1.5H8Zm0 3.5a.75.75 0 0 0 0 1.5h5a.75.75 0 0 0 0-1.5H8Z" fill="currentColor"/></svg>`,
      name: 'Lançamentos',
      path: '/transactions',
      modulo: 'lancamentos',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M3 6a3 3 0 0 1 3-3h12a3 3 0 0 1 3 3v13a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6Zm3-1a1 1 0 0 0-1 1v12h14V6a1 1 0 0 0-1-1H6Zm1 3a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm4 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm4 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0ZM7 12a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm4 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0Zm4 0a1 1 0 1 1 2 0 1 1 0 0 1-2 0ZM8 17v-2h2v2H8Zm4-2v2h2v-2h-2Z" fill="currentColor"/></svg>`,
      name: 'Fornecedores',
      path: '/suppliers',
      modulo: 'fornecedores',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M11.65 2.21a.75.75 0 0 1 .7 0l9 4.75A.75.75 0 0 1 21 8.37h-1.25v8.13h1a.75.75 0 0 1 0 1.5h-1v.25h1.5a.75.75 0 0 1 0 1.5h-18a.75.75 0 0 1 0-1.5h1.5V18h-1a.75.75 0 0 1 0-1.5h1V8.37H3a.75.75 0 0 1-.35-1.41l9-4.75ZM6.25 8.37v8.13h2.5V8.37h-2.5Zm4 0v8.13h3.5V8.37h-3.5Zm5 0v8.13h2.5V8.37h-2.5ZM18.25 18H5.75v.25h12.5V18ZM12 3.72 5.98 6.87h12.04L12 3.72Z" fill="currentColor"/></svg>`,
      name: 'Contas Bancárias',
      path: '/accounts',
      modulo: 'contas',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M11.65 2.86a.75.75 0 0 1 .7 0l8.5 4.5a.75.75 0 0 1 0 1.33l-8.5 4.5a.75.75 0 0 1-.7 0l-8.5-4.5a.75.75 0 0 1 0-1.33l8.5-4.5ZM4.85 8.02 12 11.8l7.15-3.78L12 4.24 4.85 8.02ZM2.51 12.6a.75.75 0 0 1 1.02-.3L12 16.8l8.47-4.5a.75.75 0 1 1 .71 1.33l-8.83 4.69a.75.75 0 0 1-.7 0l-8.83-4.69a.75.75 0 0 1-.31-1.02Zm1.02 3.7a.75.75 0 0 0-.71 1.33l8.83 4.69a.75.75 0 0 0 .7 0l8.83-4.69a.75.75 0 1 0-.71-1.33L12 20.8l-8.47-4.5Z" fill="currentColor"/></svg>`,
      name: 'Categorias',
      path: '/categories',
      modulo: 'categorias',
    },
    {
      name: 'Clientes',
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M9 4C7.34315 4 6 5.34315 6 7C6 8.65685 7.34315 10 9 10C10.6569 10 12 8.65685 12 7C12 5.34315 10.6569 4 9 4ZM4.5 7C4.5 4.51472 6.51472 2.5 9 2.5C11.4853 2.5 13.5 4.51472 13.5 7C13.5 9.48528 11.4853 11.5 9 11.5C6.51472 11.5 4.5 9.48528 4.5 7ZM2.5 19C2.5 15.9624 4.96243 13.5 8 13.5H10C13.0376 13.5 15.5 15.9624 15.5 19V20.25C15.5 20.6642 15.1642 21 14.75 21C14.3358 21 14 20.6642 14 20.25V19C14 16.7909 12.2091 15 10 15H8C5.79086 15 4 16.7909 4 19V20.25C4 20.6642 3.66421 21 3.25 21C2.83579 21 2.5 20.6642 2.5 20.25V19ZM16.5 6.75C16.0858 6.75 15.75 6.41421 15.75 6C15.75 5.58579 16.0858 5.25 16.5 5.25C18.7091 5.25 20.5 7.04086 20.5 9.25C20.5 11.4591 18.7091 13.25 16.5 13.25C16.0858 13.25 15.75 12.9142 15.75 12.5C15.75 12.0858 16.0858 11.75 16.5 11.75C17.8807 11.75 19 10.6307 19 9.25C19 7.86929 17.8807 6.75 16.5 6.75ZM17.75 15C17.75 14.5858 18.0858 14.25 18.5 14.25C20.8472 14.25 22.75 16.1528 22.75 18.5V19.75C22.75 20.1642 22.4142 20.5 22 20.5C21.5858 20.5 21.25 20.1642 21.25 19.75V18.5C21.25 16.9812 20.0188 15.75 18.5 15.75C18.0858 15.75 17.75 15.4142 17.75 15Z" fill="currentColor"></path></svg>`,
      path: '/costumers',
      modulo: 'clientes',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M9.5 3C7.567 3 6 4.567 6 6.5S7.567 10 9.5 10 13 8.433 13 6.5 11.433 3 9.5 3ZM4.5 6.5C4.5 3.738 6.738 1.5 9.5 1.5S14.5 3.738 14.5 6.5 12.262 11.5 9.5 11.5 4.5 9.262 4.5 6.5ZM2 19.5C2 16.462 4.462 14 7.5 14h4c3.038 0 5.5 2.462 5.5 5.5v1a.75.75 0 0 1-1.5 0v-1c0-2.21-1.79-4-4-4h-4c-2.21 0-4 1.79-4 4v1A.75.75 0 0 1 2 20.5v-1ZM17.5 7a.75.75 0 0 1 .75-.75c2.071 0 3.75 1.679 3.75 3.75s-1.679 3.75-3.75 3.75a.75.75 0 0 1 0-1.5A2.25 2.25 0 0 0 20.5 10a2.25 2.25 0 0 0-2.25-2.25A.75.75 0 0 1 17.5 7ZM18.5 15.25a.75.75 0 0 1 .75-.75c2.071 0 3.75 1.679 3.75 3.75v1.25a.75.75 0 0 1-1.5 0V18.25a2.25 2.25 0 0 0-2.25-2.25.75.75 0 0 1-.75-.75Z" fill="currentColor"/></svg>`,
      name: 'Contactos',
      path: '/contactos',
      modulo: 'contactos',
    },
  ]

  private readonly allOthersItems: NavItem[] = [
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M10.75 3.5a7.25 7.25 0 1 0 0 14.5 7.25 7.25 0 0 0 0-14.5ZM2 10.75a8.75 8.75 0 1 1 15.6 5.49l3.58 3.58a.75.75 0 1 1-1.06 1.06l-3.58-3.58A8.75 8.75 0 0 1 2 10.75Z" fill="currentColor"/></svg>`,
      name: 'Pesquisa Global',
      path: '/global-search',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 3.5C7.30558 3.5 3.5 7.30558 3.5 12C3.5 14.1526 4.3002 16.1184 5.61936 17.616C6.17279 15.3096 8.24852 13.5955 10.7246 13.5955H13.2746C15.7509 13.5955 17.8268 15.31 18.38 17.6167C19.6996 16.119 20.5 14.153 20.5 12C20.5 7.30558 16.6944 3.5 12 3.5ZM17.0246 18.8566V18.8455C17.0246 16.7744 15.3457 15.0955 13.2746 15.0955H10.7246C8.65354 15.0955 6.97461 16.7744 6.97461 18.8455V18.856C8.38223 19.8895 10.1198 20.5 12 20.5C13.8798 20.5 15.6171 19.8898 17.0246 18.8566ZM2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12ZM11.9991 7.25C10.8847 7.25 9.98126 8.15342 9.98126 9.26784C9.98126 10.3823 10.8847 11.2857 11.9991 11.2857C13.1135 11.2857 14.0169 10.3823 14.0169 9.26784C14.0169 8.15342 13.1135 7.25 11.9991 7.25ZM8.48126 9.26784C8.48126 7.32499 10.0563 5.75 11.9991 5.75C13.9419 5.75 15.5169 7.32499 15.5169 9.26784C15.5169 11.2107 13.9419 12.7857 11.9991 12.7857C10.0563 12.7857 8.48126 11.2107 8.48126 9.26784Z" fill="currentColor"></path></svg>`,
      name: 'Meu perfil',
      path: '/profile',
    },
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 20l6-8 4 4 8-10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M17 6h5v5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 20h16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>`,
      name: 'Relatórios',
      adminOnly: true,
      subItems: [
        { name: 'Fluxo Diário', path: '/fluxo-de-caixa', modulo: 'cash-flow' },
        { name: 'Clientes', path: '/relatorios/clientes', modulo: 'clientes-report' },
        { name: 'Categorias', path: '/relatorios/categorias', modulo: 'categorias-report' },
      ],
    },
  ]

  readonly navItems = computed(() => this.filterNav(this.allNavItems))

  readonly othersItems = computed(() =>
    this.filterNav(this.allOthersItems)
      .map((item) => ({
        ...item,
        subItems: item.subItems
          ? item.subItems.filter((s) => this.canSeeModulo(s.modulo))
          : undefined,
      }))
      .filter((item) => !item.subItems || item.subItems.length > 0),
  )

  openSubmenu: string | null | number = null
  subMenuHeights: { [key: string]: number } = {}
  @ViewChildren('subMenu') subMenuRefs!: QueryList<ElementRef>

  readonly isExpanded$
  readonly isMobileOpen$
  readonly isHovered$

  private subscription: Subscription = new Subscription()

  constructor(
    public sidebarService: SidebarService,
    private router: Router,
    private cdr: ChangeDetectorRef,
  ) {
    this.isExpanded$ = this.sidebarService.isExpanded$
    this.isMobileOpen$ = this.sidebarService.isMobileOpen$
    this.isHovered$ = this.sidebarService.isHovered$
  }

  private isAdmin(): boolean {
    return this.auth.currentUser()?.role === 'ADMIN'
  }

  private canSeeModulo(modulo?: string): boolean {
    if (!modulo) return true
    if (this.isAdmin()) return true
    const perms = this.auth.permissoes()
    return perms.some(
      (p) =>
        p.startsWith(modulo + '.') &&
        (p.includes('listar') ||
          p.includes('obter') ||
          p.includes('buscar') ||
          p.includes('find') ||
          p.includes('get') ||
          p.includes('resumo') ||
          p.includes('analytics')),
    )
  }

  private canSeeItem(item: NavItem): boolean {
    if (item.adminOnly) return this.isAdmin()
    if (item.modulo) return this.canSeeModulo(item.modulo)
    return true
  }

  private filterNav(items: NavItem[]): NavItem[] {
    this.auth.currentUser()
    this.auth.permissoes()
    return items.filter((item) => this.canSeeItem(item))
  }

  ngOnInit() {

    // Subscribe to router events
    this.subscription.add(
      this.router.events.subscribe((event) => {
        if (event instanceof NavigationEnd) {
          this.setActiveMenuFromRoute(this.router.url)
        }
      }),
    )

    // Subscribe to combined observables to close submenus when all are false
    this.subscription.add(
      combineLatest([
        this.isExpanded$,
        this.isMobileOpen$,
        this.isHovered$,
      ]).subscribe(([isExpanded, isMobileOpen, isHovered]) => {
        if (!isExpanded && !isMobileOpen && !isHovered) {
          // this.openSubmenu = null;
          // this.savedSubMenuHeights = { ...this.subMenuHeights };
          // this.subMenuHeights = {};
          this.cdr.detectChanges()
        } else {
          // Restore saved heights when reopening
          // this.subMenuHeights = { ...this.savedSubMenuHeights };
          // this.cdr.detectChanges();
        }
      }),
    )

    // Initial load
    this.setActiveMenuFromRoute(this.router.url)
  }

  ngOnDestroy() {
    // Clean up subscriptions
    this.subscription.unsubscribe()
  }

  isActive(path: string): boolean {
    return this.router.url === path
  }

  toggleSubmenu(section: string, index: number) {
    const key = `${section}-${index}`

    if (this.openSubmenu === key) {
      this.openSubmenu = null
      this.subMenuHeights[key] = 0
    } else {
      this.openSubmenu = key

      setTimeout(() => {
        const el = document.getElementById(key)
        if (el) {
          this.subMenuHeights[key] = el.scrollHeight
          this.cdr.detectChanges() // Ensure UI updates
        }
      })
    }
  }

  onSidebarMouseEnter() {
    this.isExpanded$
      .subscribe((expanded) => {
        if (!expanded) {
          this.sidebarService.setHovered(true)
        }
      })
      .unsubscribe()
  }

  private setActiveMenuFromRoute(currentUrl: string) {
    const menuGroups = [
      { items: this.navItems(), prefix: 'main' },
      { items: this.othersItems(), prefix: 'others' },
    ]

    menuGroups.forEach((group) => {
      group.items.forEach((nav, i) => {
        if (nav.subItems) {
          nav.subItems.forEach((subItem) => {
            if (currentUrl === subItem.path) {
              const key = `${group.prefix}-${i}`
              this.openSubmenu = key

              setTimeout(() => {
                const el = document.getElementById(key)
                if (el) {
                  this.subMenuHeights[key] = el.scrollHeight
                  this.cdr.detectChanges() // Ensure UI updates
                }
              })
            }
          })
        }
      })
    })
  }

  onSubmenuClick() {
    console.log('click submenu')
    this.isMobileOpen$
      .subscribe((isMobile) => {
        if (isMobile) {
          this.sidebarService.setMobileOpen(false)
        }
      })
      .unsubscribe()
  }
}

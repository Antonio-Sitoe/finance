...
export class CustomerApiService {
  ...
  getAllCostumers(): Observable<ICustomerDTO[]> {
    return this.http.get<ICustomerDTO[]>(CUSTOMER_API_ENDPOINTS.ALL);
  }
  ...
  updateCustomer(id: number, dto: UpdateCustomerDto): Observable<ICustomerDTO> {
    return this.http.patch<ICustomerDTO>(
      CUSTOMER_API_ENDPOINTS.UPDATE(id),
      dto
    );
  }
  ...
  updateSituacao(id: number, situacao: SITUATION): Observable<ICustomerDTO> {
    return this.http.patch<ICustomerDTO>(CUSTOMER_API_ENDPOINTS.SITUACAO(id), {
      situacao,
    });
  }
